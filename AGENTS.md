...
O service worker não pode guardar HTML de navegação nem bundles JavaScript/CSS com hash: um deploy novo remove esses caminhos e transforma o cache antigo em tela vazia. As entradas principais são estáveis (`/assets/index.js` e `/assets/index.css`) e os hashes já publicados têm rotas transitórias de compatibilidade. Cachear somente recursos públicos estáveis e a página offline; nunca `/api`, sessão ou dados. `/sw.js` deve exigir atualização (`no-store` ou `max-age=0, must-revalidate`) e ser registrado com `updateViaCache: 'none'`.
