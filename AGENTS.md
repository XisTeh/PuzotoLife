...
O service worker não pode guardar HTML de navegação nem bundles JavaScript/CSS com hash: um deploy novo remove esses caminhos e transforma o cache antigo em tela vazia. As entradas principais são estáveis (`/assets/index.js` e `/assets/index.css`), devem ser servidas com `Cache-Control: no-store`, e os hashes já publicados têm rotas transitórias de compatibilidade. Cachear somente recursos públicos estáveis e a página offline; nunca `/api`, sessão ou dados. `/sw.js` deve exigir atualização (`no-store` ou `max-age=0, must-revalidate`) e ser registrado com `updateViaCache: 'none'`.
...
- Em produção serverless, a carga inicial de uma página deve reunir seus dados relacionados em uma única rota privada quando isso eliminar autenticações e conexões repetidas. Preservar transação, snapshot e paridade com os serviços de domínio. Não pré-carregar todos os módulos de página depois do login: importar sob demanda e antecipar apenas a página indicada por foco ou ponteiro.
- O Supabase do produto está em São Paulo (`sa-east-1`) e a função Express da Vercel deve permanecer em São Paulo (`gru1`). Não remover ou trocar `regions` sem medir a latência autenticada e documentar a razão; distância entre runtime, Auth e PostgreSQL afeta todas as telas.
