...
function workerFixture({ fetchImpl = async () => ({ ok: true, clone: () => ({}) }) } = {}) {
  ...
  const sandbox = {
    ...
    caches: {
      open: async () => cache,
      keys: async () => ['puzoto-shell-v1', 'puzoto-static-v2', 'puzoto-static-v3', 'puzoto-static-v4', 'puzoto-static-v5', 'puzoto-static-v6', 'unrelated-cache'],
...
test('service worker guarda somente recursos públicos estáveis e remove o cache legado', async () => {
  ...
  await activate.done();
  assert.deepEqual(operations.deleted, ['puzoto-shell-v1', 'puzoto-static-v2', 'puzoto-static-v3', 'puzoto-static-v4', 'puzoto-static-v5']);
