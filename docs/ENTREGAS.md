...
Closes #49. A política central de HTML sanitizava fragmentos dinâmicos sem considerar o elemento de destino. Quando uma página atribuía somente `<tr>` e `<td>` ao `tbody`, o parser intermediário tratava o conteúdo como um fragmento de `body` e removia a estrutura da tabela. Os valores continuavam visíveis, mas apareciam concatenados; isso afetava tabelas em trabalho, finanças, relatórios e configurações tanto no desktop quanto no celular.
...
A varredura das 20 páginas passa a falhar se uma tabela tiver conteúdo sem linhas, número de células diferente dos cabeçalhos ou markup estrutural visível como texto. Há cobertura específica para colunas no desktop, cartões rotulados entre 320 e 390 px e uma injeção sintética que preserva cinco células enquanto bloqueia a ação inserida. Validação local: `npm run quality` aprovado com 69 testes Node, build e orçamento de 217 KB gzip; 30 jornadas Playwright aprovadas e 2 saltos previstos entre desktop e celular; `npm audit --omit=dev --audit-level=high` sem vulnerabilidades.
