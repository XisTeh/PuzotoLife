...
O `vercel.json` fixa a função em `gru1`. Um contrato automatizado impede remover silenciosamente essa localização. A mudança não altera banco, autenticação, dados, cookies ou frontend. O deploy do PR #48 publicou o commit `ea5d7de`; `x-vercel-id` confirmou `gru1::gru1`, o smoke test passou 14 verificações e a medição autenticada aquecida registrou abertura em 727 ms e navegações entre 206 e 357 ms. A primeira abertura após o deploy levou 4,4 s. Rollback: promover o deployment do commit `3b45fd6` se a função não iniciar ou a latência piorar.

## Integridade das tabelas — branch codex/table-layout-integrity

Refs #49. A política central de HTML sanitizava fragmentos dinâmicos sem considerar o elemento de destino. Quando uma página atribuía somente `<tr>` e `<td>` ao `tbody`, o parser intermediário tratava o conteúdo como um fragmento de `body` e removia a estrutura da tabela. Os valores continuavam visíveis, mas apareciam concatenados; isso afetava tabelas em trabalho, finanças, relatórios e configurações tanto no desktop quanto no celular.

A sanitização agora cria o fragmento no contexto real do destino, aplica DOMPurify e só então entrega o resultado ao setter nativo. Isso preserva linhas, células e opções sem restaurar scripts ou eventos inline. Categorias, empresas, pagadores e diagnóstico deixam de escapar o markup interno dos indicadores de estado, evitando tags exibidas como texto.

A primeira auditoria autenticada após o deploy confirmou 22 tabelas válidas nas 20 páginas e zero markup literal, mas encontrou expansão interna no desktop em Cartões, Contas a Pagar, Pessoas/Dívidas e Receitas. As grades dessas telas usavam `1fr` com mínimo automático; tabelas largas aumentavam a coluna e cortavam a lateral direita do painel. Os trilhos flexíveis agora usam mínimo zero, e cada filho de `dashboard-grid` pode encolher dentro do espaço disponível. A rolagem horizontal, quando necessária, fica contida somente no invólucro da tabela.

A varredura das 20 páginas passa a falhar se uma tabela tiver conteúdo sem linhas, número de células diferente dos cabeçalhos, markup estrutural visível como texto ou conteúdo interno maior que o painel. Há cobertura específica para colunas no desktop, cartões rotulados entre 320 e 390 px e uma injeção sintética que preserva cinco células enquanto bloqueia a ação inserida. Validação local: `npm run quality` aprovado com 69 testes Node, build e orçamento de 217 KB gzip; 30 jornadas Playwright aprovadas e 2 saltos previstos entre desktop e celular; `npm audit --omit=dev --audit-level=high` sem vulnerabilidades.

## Acabamento móvel e abertura PWA — branch codex/mobile-interface-polish

Closes #52 e #53. Os ícones instaláveis passam a usar a logotipo completa sobre uma superfície azul grafite coerente com a interface. O manifesto separa os ícones comuns da variante maskable, inclui o tamanho de 180 px para Apple e mantém a marca dentro da área segura de recorte. A abertura usa a logo transparente, wordmark, textura e um indicador de progresso sobre a paleta do Puzoto Life; ela ocupa somente o tempo da verificação real da sessão e não adiciona espera artificial.

As barras de ação de Lançamentos, Dr. Ranon e Fechamento usam classes compartilhadas. No celular, ações principais ocupam a largura útil e ficam centralizadas; pesquisa e limpeza permanecem lado a lado. Os filtros de cartões usam uma única faixa compacta, com rolagem interna apenas quando houver cartões demais. O componente `ResponsiveTables` identifica a coluna Ações, agrupa seus botões e impede que ícones da mesma ação se espalhem em linhas diferentes. Resumos de empresas em Fechamento e Relatório do Trabalho usam cartões de duas colunas com cabeçalho e valores estáveis, e o filtro do Comparativo preserva os dois campos de mês inteiros.
