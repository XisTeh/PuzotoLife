...
Correção de latência da Issue #61 (implementada localmente, ainda sem publicação): o setup de cada transação é agrupado em uma viagem PostgreSQL, mantendo `SET LOCAL`, `REPEATABLE READ READ ONLY` nas leituras e lock do acervo nas gravações. O protocolo simples executa os comandos fixos na ordem; as consultas de negócio preservam parâmetros separados ([PostgreSQL](https://www.postgresql.org/docs/current/protocol-flow.html#PROTOCOL-FLOW-SIMPLE-QUERY), [node-postgres](https://node-postgres.com/features/queries)). A validação de inicialização lê o catálogo de todas as colunas de uma vez, sem dispensar a conferência do manifesto, papel e RLS. As 31 viagens anteriores da inicialização tornam-se 5; consultas simples passam a usar 3 viagens ao todo.

Lançamentos e Dr. Ranon / RX passam a ter uma rota privada por carga inicial. Inclusões/edições nessas telas e em Gastos/Receitas podem solicitar `painel=true`: a transação grava, calcula o painel e confirma tudo antes da resposta HTTP, eliminando a segunda requisição de atualização. Se o painel falhar, a gravação também é revertida. A opção é retrocompatível para clientes existentes. Dados ficam apenas na memória da tela durante a sessão, sem cache HTTP, service worker ou armazenamento local. A medição sintética registrada em `docs/ENTREGAS.md` não substitui medição autenticada após publicação.
...
A interface está preparada como PWA instalável: manifesto, tema e service worker armazenam somente imagens públicas estáveis, manifesto e uma página offline sem dados. O ícone Apple usa 180 px; Android e demais launchers recebem versões opacas de 192 e 512 px com a logotipo completa, fundo `#111827` e área segura para recorte adaptável. Isso evita que o Android substitua a transparência por branco e esconda a parte clara da marca. HTML de navegação, bundles JavaScript/CSS e toda rota `/api` ficam fora do cache para que um deploy não deixe referências a arquivos versionados removidos. Os entrypoints estáveis `/assets/index.js` e `/assets/index.css` usam `Cache-Control: no-store`; os chunks de página continuam versionados por hash. A instalação no celular é oferecida pelo navegador quando os critérios do dispositivo são atendidos. No iPhone, também é possível usar **Compartilhar → Adicionar à Tela de Início**. A validação final deve ser feita em `https://puzoto-life.vercel.app/` depois do login do proprietário.

O splash instalado no Android é obrigatório e usa `background_color` e o mesmo ícone do aplicativo antes de qualquer HTML estar disponível. A borda opaca dos dois PNGs tem exatamente o mesmo `#111827` do fundo nativo e do painel inicial da página; a marca aparece no mesmo contexto visual enquanto os detalhes do carregamento entram, sem um cartão branco ou uma troca de cor. Uma instalação existente conserva os metadados e arquivos do WebAPK anterior; depois do deploy, é necessário remover o aplicativo instalado e instalá-lo novamente. As leituras da interface encerram em 15 segundos e as gravações em 20 segundos, mostrando erro e nova tentativa em vez de manter spinners indefinidamente.
