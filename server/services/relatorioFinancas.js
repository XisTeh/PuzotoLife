...
export function obterRelatorioFinancas(mes) {
  ...
  // RECEITAS
  // ==========================================
  const receitas_recebidas = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM receitas WHERE data LIKE ? AND status = 'recebido'`).get(`${mes}%`).total;
  ...
  // CONTAS A PAGAR
  // ==========================================
  const contas_pendentes = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM contas_pagar WHERE vencimento LIKE ? AND status = 'pendente'`).get(`${mes}%`).total;
  ...
  const contas_atrasadas = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM contas_pagar WHERE status = 'pendente' AND vencimento < ?`).get(hoje).total;

  // ==========================================
  // PESSOAS / DÍVIDAS
  // ==========================================
  let eu_devo = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM pessoas_dividas WHERE tipo = 'eu_devo' AND status = 'pendente'`).get().total;
  let me_devem = db.prepare(`SELECT COALESCE(SUM(valor), 0) as total FROM pessoas_dividas WHERE tipo = 'me_deve' AND status = 'pendente'`).get().total;
  ...
  const cartoes_por_fatura_raw = db.prepare(`
  ...
  const pessoas_lista = db.prepare(`
  ...
  const maiores_contas = db.prepare(`
  ...
  const maiores_faturas = db.prepare(`
